...
//             for (int i = 0; i < len; i++)
//             {
//                 printf("%02X ", data[i]); // Print raw data in HEX
//             }
//             printf("\n");
//         }
//         else
...
void app_main(void)
{
    ...
    while (1)
    {
        ...
        ssize_t bytes = uart_read_bytes(UART_PORT, &buffer, UART_BUFFER_SIZE - buffer_len - 1, pdMS_TO_TICKS(1000));
        ...
        if (magic_idx >= 0 && buffer_len >= magic_idx + 40)
        {
            ...
            if (buffer_len >= magic_idx + header.totalPacketLen)
            {
                ...
                // printf("Saved frame_$lu.txt with %d points and %d objects\n",
...
//     int len = uart_read_bytes(UART_PORT, &byte, 1, pdMS_TO_TICKS(200));
//     if (len > 0)
//     {
//         printf("Received: char='%c', hex=0x%02X, dec=%d\n", byte, byte, byte);
//         // printf("%s", buffer);
//     }
//     else
//     {
